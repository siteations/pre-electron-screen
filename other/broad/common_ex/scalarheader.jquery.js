 ...
 */
 (function($){
    $.scalarheader = function(el, options){
        ...
        var base = this;
		var sublink = ($('#phpVar').val());
		var suburl = ($('#phpVar2').val());
		if (sublink.trim() != '') {
			sublinkimg = '<img src="' + sublink + '"style="width:100%"></img>';
		}
		else {sublinkimg = '';}
        ...
        base.init = function(){
            ...
            var navbar_html  =  '<div class="container-fluid">'+
                                    ...
                                    '<div class="collapse navbar-collapse" id="ScalarHeaderMenu">'+
                                         '<ul class="nav navbar-nav" id="ScalarHeaderMenuLeft">'+
                                            ...
                                            '</li>'+
                                        '</ul>'+
                                        '<span class="navbar-text navbar-left pull-left title_wrapper hidden-xs" id="desktopTitleWrapper"><span class="hidden-xs author_text"> &#x9; <a href="https://www.newberry.org" style="color: #ffffff;"><img src="http://publications.newberry.org/scalar_tests/test-book/media/newberry_logo_dark.png" style="height:40px; width:130px;"></a></span></span>'+
								...
								'</div>';
			var rc_banner = '<div class="rc-banner"><a href="' + suburl + '" target="_blank">' + sublinkimg + '</a></div>';
            ...
            base.$el.append(navbar).find('.title_wrapper').append(title_link.clone());
			
			if (sublink.trim() != '') {
				$('#ScalarHeaderMenu').append(rc_banner);
			}
        ...
        base.handleResize = function(extra_offset){
			...
			var banner_width = $('.rc-banner').width();
			if (sublink.trim() != '') {
				title_width -= 520;
			}
